...
export const getTransactions = async (req, res) => {
  try {
    ...
    const transactions = await pool.query({
      text: `SELECT * FROM tableTransaction WHERE user_id = $1 AND createdat BETWEEN $2 AND $3 AND (description ILIKE '%' || $4 || '%' OR status ILIKE '%' || $4 || '%' OR source ILIKE '%' || $4 || '%') ORDER BY id DESC`,
...
export const getDashboardInformation = async (req, res) => {
  try {
    ...
    tableTransaction WHERE user_id = $1 GROUP BY type`,
      values: [userId],
    });
    ...
    const lastTransactionsResult = await pool.query({
      text: `SELECT * FROM tableTransaction WHERE user_id = $1 ORDER BY id DESC LIMIT 5`,
      values: [userId],
    });
    ...
    const lastAccountResult = await pool.query({
      text: `SELECT * FROM tableAccount WHERE user_id = $1 ORDER BY id DESC LIMIT 4`,
      values: [userId],
    });
...
export const addTransaction = async (req, res) => {
  try {
    ...
    const result = await pool.query({
      text: `SELECT * FROM tableAccount WHERE id = $1`,
      values: [account_id],
    });
    ...
    await pool.query({
      text: `UPDATE tableAccount SET account_balance = account_balance - $1, updatedat = CURRENT_TIMESTAMP WHERE id = $2`,
      values: [amount, account_id],
    });

    await pool.query({
      text: `INSERT INTO tableTransaction(user_id, description, type, status, amount, source) VALUES($1, $2, $3, $4, $5, $6)`,
