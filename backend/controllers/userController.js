...
export const getUser = async (req, res) => {
  try {
    ...
    const userExist = await pool.query({
      text: `SELECT * FROM tableUser WHERE id = $1`,
      values: [userId],
    });
...
export const changePassword = async (req, res) => {
  try {
    ...
    await pool.query({
      text: `UPDATE tableUser SET password = $1 WHERE id = $2`,
...
export const updateUser = async (req, res) => {
  try {
    ...
    const updatedUser = await pool.query({
      text: `UPDATE tableUser SET firstname = $1, lastname = $2, country = $3, currency = $4, contact = $5, updatedat = CURRENT_TIMESTAMP WHERE id = $6 RETURNING *`,
